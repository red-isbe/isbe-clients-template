...
export default function Nodes({ config }: IProps) {
  ...
  const nodeInfoHandler = useCallback(
    async (name: string) => {
      console.log("nodeInfoHandler called for node:", name);
      const needle: QuorumNode = getDetailsByNodeName(config, name);
      console.log("Node details:", needle);
      await axios({
        method: "POST",
        url: `${publicRuntimeConfig.QE_BASEPATH}/api/nodeGetDetails`,
        headers: {
        ...
        .then((res) => {
          console.log("API call successful, data:", res.data);
          setNode({
            selectedNode: name,
            client: needle.client,
      ...
      await axios({
        method: "POST",
        url: `${publicRuntimeConfig.QE_BASEPATH}/api/nodeGetPeers`,
        headers: {
          "Content-Type": "application/json",
        },
        data: JSON.stringify({
